...
但是，我们在用户态的程序不能直接通过`bl`来调用内核的函数。我们在[操作系统](https://evian-zhang.github.io/learn-assembly-on-Apple-Silicon-Mac/./4-操作系统.html)中提到，AArch64有不同的异常级别。一般来说，用户态程序的异常级别是EL0，内核处于EL1。低异常级别的程序是不能调用高异常级别的函数的。
...
[系统调用号我们可以在xnu源码](https://github.com/apple-oss-distributions/xnu)的`bsd/kern/syscalls.master`文件中查看。例如：
...
[因此，我们终于可以用C语言写一个Hello world了（源代码位于codes/13-hello-world.s](https://github.com/Evian-Zhang/learn-assembly-on-Apple-Silicon-Mac/blob/master/codes/13-hello-world.s)）：
...
_main:
    ...
    mov    w2, #11                      ; nbyte: 11 (length of "Hello world")
